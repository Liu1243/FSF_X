...
# ============================================================
# FSF_X: HMamba + Occ + OTA 三创新点组合配置（mini-nuscenes）
# 检测器：FSF_X   (继承链: FSF_X → FSF_HMamba → FSF)
# 特征交互：H-Mamba 跨模态序列化
# 视锥增强：FrustumOccFilter 占据引导非模态补全
# 标签分配：OTAAssigner (Sinkhorn-Knopp 最优传输)
# ============================================================
...
_occ_filter_in_ch = 67 + 64  # VoteSegHead: seg_logits(67) + seg_feats(64)
...
segmentor = dict(
    ...
    middle_encoder=dict(type='PseudoMiddleEncoderForSpconvFSD'),
    backbone=dict(
...
model = dict(
    ...
    # ── 创新点1：H-Mamba 跨模态交互 ──────────────────────────────────────
    hmamba_cfg=dict(
        d_model=1024,       # 与 embed_dims 一致
        d_state=16,         # SSM 状态空间维度
        expand_factor=2,    # 内部维度 = 2 × d_model
        dt_rank='auto',
        conv_kernel=4,
        use_fast_path=False,
    ),
    ...
    bbox_head=dict(
        ...
        loss_cls=dict(type='FocalLoss', use_sigmoid=True, gamma=4.0, alpha=0.25, loss_weight=1.0),
        ...
        tasks=tasks,
        class_names=class_names,
        common_attrs=dict(center=(3, 2, 128), dim=(3, 2, 128), rot=(2, 2, 128), vel=(2, 2, 128)),
        num_cls_layer=2,
        cls_hidden_dim=128,
        separate_head=dict(type='FSDSeparateHead', norm_cfg=dict(type='LN'), act='gelu'),
    ),
    ...
    cluster_assigner=dict(
        cluster_voxel_size=[
            (0.3, 0.3, 8), (0.3, 0.3, 8), (0.3, 0.3, 8),
            (0.1, 0.1, 8), (0.2, 0.2, 8), (0.05, 0.05, 8),
        ],
        min_points=2,
        point_cloud_range=point_cloud_range,
    ...
    frustum_obj_head=dict(
        ...
        assigner=dict(
            ...
            assigner_3d=dict(type='PointInBoxAssigner'),
            class_names=class_names,
            tasks=tasks,
        ),
        ota_assigner=ota_assigner,
        use_ota=True,
        loss_cls=dict(type='FocalLoss', use_sigmoid=True, gamma=4.0, alpha=0.25, loss_weight=1.0),
        loss_center=dict(type='L1Loss', loss_weight=0.5),
        loss_size=dict(type='L1Loss', loss_weight=0.5),
        ...
        train_cfg=dict(),
        test_cfg=dict(
            use_rotate_nms=True, nms_pre=-1, nms_thr=0.35,
            score_thr=0.01, min_bbox_size=0, max_num=500,
        ),
        norm_cfg=dict(type='LN'),
        tasks=tasks,
        class_names=class_names,
    ...
    refined_obj_head=[
        dict(
            ...
            assigner=dict(
                ...
                assigner_3d=dict(type='PointInBoxAssigner', extra_height=0.0),
                assigner_dist=dict(
                    type='DistAssigner',
                    assign_tasks=[
            ...
            ota_assigner=ota_assigner,
            use_ota=True,
            loss_cls=dict(type='FocalLoss', use_sigmoid=True, gamma=4.0, alpha=0.25, loss_weight=2.0),
            loss_center=dict(type='L1Loss', loss_weight=0.5),
            loss_size=dict(type='L1Loss', loss_weight=0.5),
            loss_rot=dict(type='L1Loss', loss_weight=0.2),
            loss_vel=dict(type='L1Loss', loss_weight=0.2),
