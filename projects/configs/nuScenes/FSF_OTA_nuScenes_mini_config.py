...
ota_assigner = dict(
    ...
    lambda_giou=2.0,
    lambda_center=1.0,
    eps=0.1,               # entropy regularisation strength
    sinkhorn_iters=50,     # Sinkhorn-Knopp iterations
    match_score_thre=0.0,  # min π score to accept as positive
    bg_col_cost=0.0,       # virtual background column cost
    candidate_topk=13,     # per-GT candidate pool size (-1 = all)
)
...
model = dict(
    ...
    # ── Camera frustum head: FSFOTAHead with OTA assigner ──────────────
    frustum_obj_head=dict(
        ...
        # OTA assigner replaces HybridAssigner
        assigner=dict(
            type='HybridAssigner',
            num_cams=6,
            assigner_2d=dict(
    ...
    # ── Refined head: FSFOTAHead with OTA assigner ─────────────────────
    refined_obj_head=[
        dict(
            type='FSFOTAHead',
            num_classes=num_classes,
            ...
            # Keep FrustumAssigner for the initial assignment stage;
            # OTA refines the assignment on top via get_targets_single override.
            assigner=dict(
